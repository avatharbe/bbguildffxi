...
## 2.1.0 22/09/2026
  - [NEW] Added a full EPV/unit/functional/smoke/integration test suite (#4)
  - [NEW] Added `specialization_provider_interface` (no-op: Final Fantasy XI jobs are already terminal, no sub-spec layer to seed) (#5)
  - [FIX] Various CI/test fixture fixes surfaced while adding the new suite (smallint overflow in the guild fixture id, missing `bb_portal_tabs` seed row in the guild-view functional fixture)
  - [FIX] `cleanup.sql`: stale `bbguild_ffxi` naming and a wrong column name
...
## 2.0.0-rc1 24/07/2026
  ...
  - [CHG] Namespace/composer/repo dropped to no-separator form (`bbguildffxi`); DB-stored config keys (`bbguild_ffxi_version`) preserved with the original underscore form
  - [CHG] Version tracking moved out of `phpbb_config` into `ext::BBGUILDFFXI_VERSION`
  ...
  - [DOCS] README: fixed wrong GitHub org (bbGuild Core / Issue Tracker links pointed at `avandenberghe/bbguild` instead of `avatharbe/bbguildffxi`), stale PHP >= 7.4.0 requirement (actual has been 8.1.0)
