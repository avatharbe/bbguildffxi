<?php
/**
 * bbGuild FFXI Extension — extension lifecycle smoke test
 *
...
/**
 * Disables and re-enables bbguildffxi and asserts the enable/disable HTTP
 ...
 * @group smoke
 */
class avathar_bbguildffxi_smoke_extension_lifecycle_test extends phpbb_functional_test_case
{
	...
	public function test_disable_and_reenable_bbguildffxi()
	{
		$this->assertTrue($this->is_active('avathar/bbguildffxi'), 'bbguildffxi should already be active from setup_extensions()');

		$this->disable_ext('avathar/bbguildffxi');
		$this->assertFalse($this->is_active('avathar/bbguildffxi'));

		$this->install_ext('avathar/bbguildffxi');
		$this->assertTrue($this->is_active('avathar/bbguildffxi'));
	}
}
